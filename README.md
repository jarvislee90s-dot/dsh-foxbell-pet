...
> **v2.1.0 面向 dsh ≥ 0.1.2-rc.1**（rc.7/rc.8 等旧版不再支持，见下方兼容表；**0.1.5-rc.2 已逐项自检兼容**）。
...
2. **五口径迷你条** —— 悬停宠物 ≈0.5s 浮现：节奏表盘（当前档位读数）+「今日 × · 本会话 ×」五口径用量 +「N 等审批 / N 运行 / N 完成」状态行；**纯只读**，移开即消失（右键「🏷 今日用量」可手动唤出，点外部 / ESC 关）。
3. **警报举牌 + 语音三优先级** —— 日阈值 `dayLimitTokens` 达 80% / 100% 各举牌一次「今日已消耗 X」，里程碑过 `milestoneUnit` 整数关口气泡一句话；警报语音三优先级：**语音组音频 > TTS 朗读 > 静默**（`ttsEnabled` 可开）。当前内置与外部宠物清单均无 usage 语音组，阈值/里程碑警报实际走 TTS / 静默。
