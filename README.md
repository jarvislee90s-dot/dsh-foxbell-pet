...
- **导出分享**：720×1600 竖版卡片（对齐 Codex++ 排版）——标题/日期范围、累计 Token hero（完整千分位）、五口径指标行、趋势卡（峰值+渐变折线）、按 Model 分布（**模型全名**+进度条）、工具调用 2×2；最下方为聊天式**评语气泡 + 宠物立绘**（姿态随机或自选，气泡不遮挡宠物）。评语来自规则池（zh/en 优先级匹配），或经头部「**按自定义评语导出**」内联填写模板（`{range}` `{tokens}` `{hitPct}` `{models}`）；「复制文本」为同数据纯文本摘要。
