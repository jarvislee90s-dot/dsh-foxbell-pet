...
var ZH = {
  ...
  "dash.weekHit": "\u672C\u5468\u547D\u4E2D {a}\uFF08\u4E0A\u5468 {b} \xB7 {d}\uFF09",
...
var EN = {
  ...
  "dash.weekHit": "This week {a} (last week {b} \xB7 {d})",
...
function weekHit(trend, weekIdx) {
  const days = trend && Array.isArray(trend.days) ? trend.days : [];
  if (days.length === 0) return 0;
  const anchorKey = days.reduce((a, b) => b.key > a.key ? b : a, days[0]).key;
  const anchorMs = (/* @__PURE__ */ new Date(anchorKey + "T00:00:00")).getTime();
  const dow = new Date(anchorMs).getDay();
  const weekStartMs = anchorMs - (dow + 6) % 7 * 864e5;
  const startMs = weekStartMs - weekIdx * 7 * 864e5;
  const endMs = weekIdx === 0 ? anchorMs : weekStartMs - 864e5;
  let req = 0, hit = 0;
  for (const d of days) {
    const ms = (/* @__PURE__ */ new Date(d.key + "T00:00:00")).getTime();
    if (ms >= startMs && ms <= endMs) {
      req += d.requestTotal || 0;
      hit += d.cacheRead || 0;
    }
  }
  return req > 0 ? hit / req : 0;
}
function hitDeltaText(trend) {
  const d = (weekHit(trend, 0) - weekHit(trend, 1)) * 100;
  return (d > 0 ? "+" : d < 0 ? "-" : "\xB1") + Math.abs(d).toFixed(1) + "pt";
}
...
function DashboardPanel() {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "dyn-pet-dash", children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime16.jsxs)("div", { className: "dyn-pet-dash-hero", children: [
      ...
      (tab === "5h" || tab === "7d") && trend ? /* @__PURE__ */ (0, import_jsx_runtime16.jsx)("div", { className: "dyn-pet-dash-weekhit", children: t("dash.weekHit", { a: fmtPct(weekHit(trend, 0)), b: fmtPct(weekHit(trend, 1)), d: hitDeltaText(trend) }) }) : null
    ] }),
...
.dyn-pet-dash { padding: 20px 28px; overflow: auto; height: 100%; box-sizing: border-box; color: #3b2f23; font-size: 14px; line-height: 1.55; max-width: 1020px; margin: 0 auto; }
.dyn-pet-dash-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; flex-wrap: wrap; }
.dyn-pet-dash-head h2 { margin: 0; font-size: 17px; font-weight: 700; color: #7a4a2b; }
...
.dyn-pet-dash-row span { font-size: 14px; color: #6b7280; }
.dyn-pet-dash-row strong { font-size: 14px; font-weight: 600; color: #1f2937; font-variant-numeric: tabular-nums; }
...
.dyn-pet-dash-model { display: flex; align-items: center; gap: 12px; padding: 5px 0; font-size: 14px; }
...
.dyn-pet-dash-toolcell span { font-size: 12px; color: #6b7280; }
.dyn-pet-dash-toolcell strong { font-size: 15px; font-weight: 600; color: #1f2937; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
