...
function boardTokensLine(s) {
  const tk = s.tokens;
  const pct = tk.requestTotal > 0 ? tk.hitPct.toFixed(1) : "0.0";
  return t("dash.requestInput") + " " + fmtTokens(tk.requestTotal) + t("dash.boardHitPct", { inner: t("dash.cacheHit") + " " + fmtTokens(tk.cacheRead) + " \xB7 " + pct + "%" }) + " " + t("dash.output") + " " + fmtTokens(tk.output) + " \xB7 " + t("dash.yourInput") + " ~" + fmtTokens(tk.userEst) + t("dash.estimateSuffix") + " \xB7 " + t("dash.withSubagents");
}
function boardRows(s) {
  return [
    t("dash.boardSessions") + " " + s.sessions + " \xB7 " + t("dash.boardTurns") + " " + s.turns + " \xB7 " + t("dash.boardErrors") + " " + s.errors,
    t("dash.boardTodayToken") + " " + boardTokensLine(s),
    t("dash.boardToolsTop") + " " + (s.toolRows.length ? s.toolRows.map(toolEntry).join(" \xB7 ") : "\u2014"),
    t("dash.boardLongest") + " " + fmtLongest(s.longest ? s.longest.ms : 0)
    // null → 「0 秒」桶（与 zh longestText 同口径）
  ];
}
...
function Board(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime5.jsxs)("div", { className: "dyn-pet-board" + (props.mode === "farewell" ? " farewell" : ""), children: [
    ...
    boardRows(s).map((text, i) => /* @__PURE__ */ (0, import_jsx_runtime5.jsx)("div", { className: "dyn-pet-board-row", children: text }, i)),
