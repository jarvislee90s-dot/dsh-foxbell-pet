    ...
    function Pet(props) {
      ...
      React.useEffect(() => {
        ...
        const applyProjects = (list) => {
          ...
          // 出错 → 委屈动画 + error 组语音（若有）+ 字幕
          if (errTitle) {
            const v = pickVoice('error')
            if (v) playVoice(v, '呜… ' + errTitle + ' 报错了', 'failed')
            else { showBubble('呜… ' + errTitle + ' 报错了', 3000); playTransient('failed', 2500) }
          }
        ...
        const handleCompletions = (list) => {
          ...
          if (!v) { setBubble(text); return }
          playVoice(v, text, 'jumping')
        }
