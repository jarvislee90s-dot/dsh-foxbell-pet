...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Pet(props) {
      ...
      // 关宠分发：隐藏桌宠时若有最近总结 → farewell 黑板（宠物隐藏后黑板仍渲染，ttl 到点自动消失）
      React.useEffect(() => petStore.subscribe(() => {
        ...
        if (petStore.visible === false && cfgRef.current.summaryEnabled && dashUiRef.current && dashUiRef.current.summary) {
          openBoard('farewell') // ttl 统一经 openBoard；effect 首挂载后才执行，捕获首渲染绑定（cfgRef/later 均稳定，同下方 later 用法）
        }
      }), [])
      React.useEffect(() => {
      ...
      // 定时器助手：登记进 timersRef（卸载时统一清理），返回可取消的 dispose；须声明在 openBoard 等消费者之前（const 箭头函数不提升）
      const later = (fn, ms) => {
      ...
      // 小黑板（组件④）：null 关闭 / manual 菜单或入口打开 / farewell 关宠分发；黑板开着不渲染迷你条（黑板优先）
      const [board, setBoard] = React.useState(null)
      const boardTimerRef = React.useRef(null) // 黑板 ttl 定时器：所有打开路径统一走 openBoard，防跨模式泄漏/堆叠
      const [entry, setEntry] = React.useState(false) // 📖 限时总结入口
      const entryTimerRef = React.useRef(null) // 入口 ttl 定时器：新完成事件先清旧定时器，防堆叠后最早到期误关
      const boardRef = React.useRef(null)
      const petRootRef = React.useRef(null)
      const miniOpen = () => setMiniMode('manual')
      // 开黑板（spec ④：黑板出现→停留 boardTtlSec→自动消失，对 manual/farewell 一视同仁）：先清旧 ttl 再按当前配置重设
      const openBoard = (mode) => {
        if (boardTimerRef.current) { const d = boardTimerRef.current; boardTimerRef.current = null; try { d() } catch {} }
        setBoard({ mode })
        boardTimerRef.current = later(() => { boardTimerRef.current = null; setBoard(null) }, (cfgRef.current.boardTtlSec || 15) * 1000)
      }
      // 关黑板（✕；ESC/点外由 Task 14 收口时也必须走这里）：同时取消 ttl 定时器
      const closeBoard = () => { if (boardTimerRef.current) { const d = boardTimerRef.current; boardTimerRef.current = null; try { d() } catch {} } setBoard(null) }
      const boardOpen = () => openBoard('manual')
      React.useEffect(() => {
      ...
      // 统一 ESC 分层关闭 + 点外部关闭（spec 6.6 规则 8）：ESC 一次剥一层（手动迷你条 → 黑板 → 菜单）；
      // 手动迷你条/黑板开时点外部关闭（宠物本体/迷你条/黑板/菜单内部点击不关）。黑板关闭必须走 closeBoard（同时取消 ttl 定时器）
      React.useEffect(() => {
        const onKey = (e) => {
      ...
      React.useEffect(() => {
        ...
        const handleCompletions = (list) => {
          ...
          // 📖 限时入口：任何有效完成事件都触发（不依赖语音是否存在，spec 6.4）；先清旧 ttl 定时器防堆叠（否则入口在首个事件的截止时刻提前消失）
          if (cfgRef.current.summaryEnabled) {
            if (entryTimerRef.current) { const d = entryTimerRef.current; entryTimerRef.current = null; try { d() } catch {} }
            setEntry(true)
      ...
      const onPointerDown = (e) => {
        clearHoverTimer(); if (miniMode !== 'manual') setMiniMode(null) // 拖拽即隐藏（手动模式除外）
      ...
      // 滚轮穿透（spec 6.6 规则 7）：宠物本体上滚动 → 暂时摘掉自身 pointer-events，用 elementFromPoint 找到下方元素，
      // 把滚动量转给其最近可滚动祖先（无则落到页面滚动元素）。黑板/菜单是独立 fixed 元素不经过此 handler，内部滚动天然正常
      ...
      return React.createElement(React.Fragment, null,
        visible ? React.createElement('div', {
          ...
          // 📖 限时入口：挂主 root 内 sprite 右上（.dyn-pet-entry 的 absolute 偏移以 192×208 root 为锚，spec 6.0/6.4 宠物旁限时出现）；点击展开黑板并自毁
        ...
        menu !== null ? React.createElement('div', {
          ...
          (menuPage && ACTION_PAGE_FIELD[menuPage])
            ...
            : menuPage === 'About' ? React.createElement(React.Fragment, null,
              React.createElement('div', { className: 'dyn-pet-menu-item', onClick: () => setMenu(null) }, 'dsh-foxbell-pet v1.4.0'),
        ...
        // 小黑板：fragment 层渲染（farewell 要在宠物隐藏后仍显示）；无 summary 时 Board 自身返回 null
        board !== null ? React.createElement('div', { ref: boardRef, style: { position: 'fixed', right: 24, bottom: 76, zIndex: 2147483000 } },
          React.createElement(Board, { dash: dashUiRef.current, mode: board.mode, ttlSec: cfg.boardTtlSec, onClose: closeBoard })) : null,
      )
    }
    ...
    // 小黑板（组件④）：宿主拼好的 dashboard.summary 纯展示；无 summary 返回 null（无数据不弹）。
    // 关闭四路：✕ 在此、点外部/ESC 由 Task 14 统一收口、自动消失由调用方 ttl 定时器负责
    ...
    function MiniBar({ dash, projects, usageOn }) {
      ...
      const day = u.day || {}
      // 五口径（2026-09-10 用户裁定）：请求输入=全文累计（uncached+命中）；命中/未命中/产出为真值；用户输入为宿主启发式估算
      const reqTotal = (u.requestTotal !== undefined) ? u.requestTotal : (day.inputTokens || 0) + (day.cacheReadTokens || 0)
      const hit = (u.cacheHitRate !== undefined) ? u.cacheHitRate : 0
      const sess = u.session
      const sessReq = sess ? ((sess.requestTotal !== undefined) ? sess.requestTotal : (sess.inputTokens || 0) + (sess.cacheReadTokens || 0)) : null
    ...
    function adoptStyles() {
      ...
      style.textContent = `
        ...
        /* 卡片壳逐值对齐宿主 PluginCard.module.css（含 dsw 设计变量，视觉与原生卡 1:1） */
        ...
        .dyn-pet-card:hover { border-color: var(--dsw-alias-label-dimmed, rgba(0,0,0,.2)); }
        .dyn-pet-card.dyn-pet-card-open { background: var(--dsw-alias-bg-layer-2, #fff); border-color: var(--dsw-alias-label-dimmed, rgba(0,0,0,.2)); }
        .dyn-pet-card-head { all: unset; box-sizing: border-box; display: flex; align-items: center; gap: 12px; width: 100%; padding: 14px 16px; cursor: pointer; font: inherit; color: inherit; text-align: left; border-radius: 12px; }
        .dyn-pet-card-head:focus-visible { outline: 2px solid var(--dsw-alias-brand-primary, #4f7cff); outline-offset: -2px; }
        .dyn-pet-card-titles { flex: 1; min-width: 0; display: flex; flex-direction: column; align-items: flex-start; gap: 4px; }
        .dyn-pet-card-title { font-size: 15px; font-weight: 600; line-height: 1.4; color: var(--dsw-alias-label-primary, #333); }
        .dyn-pet-card-desc { font-size: 13px; line-height: 1.5; color: var(--dsw-alias-label-tertiary, #999); }
        .dyn-pet-card-chevron { flex: none; color: var(--dsw-alias-label-tertiary, #999); font-size: 14px; transition: transform .16s; }
        .dyn-pet-card-chevron.open { transform: rotate(180deg); }
        .dyn-pet-settings { border-top: 0.5px solid var(--dsw-alias-border-l2, rgba(0,0,0,.08)); margin: 0 16px; padding: 0 0 8px; }
    ...
    function SettingsCard(props) {
      // 卡片壳（照宿主 PluginCard 模式）：一级标题 + 默认收起 + 头部未保存标记 + 保存成功自动收起。
      ...
      const save = () => {
        ...
        setSavedFlash(true)
        setOpen(false) // 保存结算成功自动收起（宿主 PluginCard 同款行为）
      }
      const discard = () => { dirtyRef.current = false; setDraft(cfg) }
      // 宿主把本槽渲染在 <ul class="cards"> 内，根元素用 li 才合规
      return React.createElement('li', { className: 'dyn-pet-card' + (open ? ' dyn-pet-card-open' : '') },
        React.createElement('button', {
          type: 'button',
          className: 'dyn-pet-card-head',
          onClick: () => setOpen(!open),
          'aria-expanded': open,
        },
          React.createElement('span', { className: 'dyn-pet-card-titles' },
            React.createElement('span', { className: 'dyn-pet-card-title' }, 'Foxbell 宠物'),
            React.createElement('span', { className: 'dyn-pet-card-desc' }, '桌宠行为与效率看板'),
          ),
          ...
          React.createElement('span', { className: 'dyn-pet-card-chevron' + (open ? ' open' : '') }, '▾'),
        ),
        open ? React.createElement('div', { className: 'dyn-pet-settings' },
