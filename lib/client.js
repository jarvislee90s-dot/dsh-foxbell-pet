...
async function exportDashboardImage(input) {
  ...
  const bubbleLines = (() => {
    ...
    for (const ch of input.quote) {
      if (ch === "\n" || c.measureText(line + ch).width > bubbleW - 44) {
        lines.push(line);
        line = ch === "\n" ? "" : ch;
      } else line += ch;
    }
...
var POOL = {
  zh: [
    ...
    { when: (a) => a.total < 1e5, text: "\u4ECA\u5929\u5F88\u8282\u5236\uFF0C\u5C3E\u5DF4\u90FD\u7FD8\u8D77\u6765\u4E86\u3002" },
  ...
  en: [
    ...
    { when: (a) => a.total < 1e5, text: "Lean day. Tail's up!" },
...
function DashboardPanel() {
  ...
  const buildQuote = (custom2) => pickQuote({ total: hero, hitPct: hitRate, trendUp, multiModel: models.length > 1, loaf }, getLang(), custom2, quoteVars);
