...
function foldUsage(evts) {
  ...
  for (const ev of Array.isArray(evts) ? evts : []) {
    ...
    if (key === lastKey && lastBuckets && lastDay) apply2(lastBuckets, lastDay, -1);
    apply2(b, day, 1);
    lastKey = key;
    lastBuckets = b;
    lastDay = day;
...
function evaluateAlerts(prev, next, cfg, dateKey) {
  ...
  if (limit > 0) {
    ...
    if (prev.dayTotal < warn && next.dayTotal >= warn) {
      out.push({ id: "day-warn" + day, kind: "day-warn", text: "\u4ECA\u65E5 token \u5DF2\u7528 80% \xB7 " + formatTokens(next.dayTotal) });
    }
    if (prev.dayTotal < limit && next.dayTotal >= limit) {
      out.push({ id: "day-hit" + day, kind: "day-hit", text: "\u4ECA\u65E5 token \u5DF2\u8FBE\u9608\u503C \xB7 " + formatTokens(next.dayTotal) });
    }
  }
...
var events5mCount = (evts, nowMs) => evts.filter((e) => e && typeof e.time === "number" && e.time > nowMs - 3e5 && ACTIVITY_TYPES.has(e.type)).length;
var BOARD_DEFAULTS = {
  paceEnabled: true,
...
function buildDashboard(sessionsData, cfg, now, alertPrev) {
  ...
  const compute = () => {
    ...
    for (const a of roots) {
      ...
      try {
        ...
        if (session) {
          const sessionEvts = sessionEvents(session);
