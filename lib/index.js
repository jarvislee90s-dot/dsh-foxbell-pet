...
export async function apply(ctx, config) {
  ...
  installSettingsSection(ctx, FOXBELL_PET_NS, Config, entry, {
    setSource: () => {},
    onChange: () => {},
  })
  ...
  const computeProjects = () => {
    ...
    for (const a of roots) {
      ...
      if (m) {
        if (m.lastEventTime !== null) {
          ...
          agg5m += events5mCount(sessionEvts || [], m.lastEventTime)
        }
    ...
    if (entry.paceEnabled) {
      paceState = derivePaceTier({ turnOpen: aggTurnOpen, lastEventTime: aggLastEvent, eventCount5m: agg5m }, now, {
        intenseEvents: entry.paceIntenseEvents,
        longrunSilentMs: entry.paceLongrunMin * 60000,
        loafStartMs: entry.paceLoafStartMin * 60000,
      })
    }
    const dayTotal = dayUsage.inputTokens + dayUsage.outputTokens + dayUsage.cacheReadTokens + dayUsage.cacheWriteTokens
    const newAlerts = entry.usageEnabled
      ? evaluateAlerts(alertPrev, { dayTotal, grandTotal }, { dayLimitTokens: entry.dayLimitTokens, milestoneUnit: entry.milestoneUnit })
      : []
